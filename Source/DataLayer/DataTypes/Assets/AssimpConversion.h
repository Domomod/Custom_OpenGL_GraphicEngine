...
static inline glm::vec4 glmCast(const aiColor4D &v) { return glm::vec4(v.r, v.g, v.b, v.a); }
static inline glm::vec3 glmCast(const aiVector3D &v) { return glm::vec3(v.x, v.y, v.z); }
static inline glm::vec2 glmTexCoordCast(const aiVector3D &v) { return glm::vec2(v.x, 1 - v.y); } // it's aiVector3D because assimp's texture coordinates use that
