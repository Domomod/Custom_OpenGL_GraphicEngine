                                            ...
                                            const std::map<std::__cxx11::string, int> &boneNameToIndexMap)
{
    ...
    for (int i = 0; i < assimpAnimation->mNumChannels; i++)
    {
        ...
        constructedAnimation->idToBoneAnimMap.emplace(
                boneIndex,
                SkeletalSystem::BoneAnimation(
                        translateKeyframes<SkeletalSystem::KeyFrame<glm::vec3>>(nodeAnimation->mNumPositionKeys,
...
std::shared_ptr<SkeletalSystem::SkeletalAnimation> SkeletonAnimationLoader::make()
{
    ...
    {
        throw AnimationLoadingException(
                "Tried to create an empty animation. Possibly tried to return the same animation twice.");
    }
