...
void main() {
    ...
    vec3 N = normalize(texture(normalMap, fs_in.fragTexCoord).rgb * 2.0 - 1.0);
    N      = normalize(fs_in.TBN * N);
    vec3 V = normalize(fs_in.viewerPos_world - fs_in.fragPos_world);
    vec3 L = normalize(fs_in.lightPos_world  - fs_in.fragPos_world);
    vec3 H = normalize(V + L);

    float dist     = length(fs_in.lightPos_world - fs_in.fragPos_world);
    ...
    float NDF   = calculateNDF_GGX(N, H, roughness);
